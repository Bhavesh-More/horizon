...
Monorepo (workspaces config: Yarn workspaces via a `workspaces` field in `package.json` or `yarn.lock`, `turbo.json`, `nx.json`, `lerna.json`; or multiple manifests under `apps/*` / `packages/*`): plan per workspace, never mix apps:
