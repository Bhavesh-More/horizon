...
Monorepo: run the specific affected app, not the repo root. Find the workspace the change lives in (`apps/<x>/…`) and use its run command (e.g. `yarn workspace <x> dev`, `turbo run dev --filter=<x>`, or that workspace's `package.json` script). A change to a shared package: run the app(s) that consume it.
