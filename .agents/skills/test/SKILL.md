...
Yes: install with the project's package manager (`yarn` shown; substitute the detected npm/yarn/bun, or the language's manager for Python/Go):

```bash
yarn add -D vitest                                            # unit
yarn add -D @testing-library/react @testing-library/user-event @testing-library/jest-dom  # addon
yarn add -D @playwright/test && npx playwright install        # E2E (Playwright)
yarn add -D cypress                                            # E2E (Cypress)
...
- Read `package.json`, note `scripts.test`. `RUN_COMMAND` = `<pkgmgr> test` when a `test` script exists (`<pkgmgr> run test` for npm); a raw invocation (e.g. `npx vitest run` or `yarn vitest run`) only when none does.
