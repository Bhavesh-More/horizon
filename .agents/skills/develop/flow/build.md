...
**Monorepo: work inside the target workspace.** Workspaces config or `apps/*`/`packages/*` manifests → identify the feature's workspace (code pointer, or task path) and operate there: its nested `AGENTS.md` and `design.md`, its `package.json`/stack, write into its tree, run its commands (`dev`/`build`/`test`, e.g. `yarn workspace <workspace> …` or `turbo run … --filter`). The checks before you start apply to that workspace; its scope is `docs/scope/<workspace>/`.
