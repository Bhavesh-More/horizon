...
Monorepo: keep root to monorepo wide concerns (workspace tooling `yarn`/`turbo`/`nx`, shared standards, a `## Context files` section pointing at each workspace's nested doc); per app stack does not go in root.
